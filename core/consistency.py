...
class ConsistencyChecker:
    ...
    @staticmethod
    def check_against_preset(probe_result, preset_thresholds):
        """检查单个文件的参数是否符合预设标准（保留兼容）"""
        issues = []
        video = probe_result.get("video")
        audio = probe_result.get("audio")
        if not preset_thresholds:
            return issues

        if video and "video" in preset_thresholds:
            v_std = preset_thresholds["video"]
            checks = [
                ("codec", "视频编码", v_std.get("codec")),
                ("width", "宽度", v_std.get("width")),
                ("height", "高度", v_std.get("height")),
                ("fps", "帧率", v_std.get("fps")),
                ("pix_fmt", "像素格式", v_std.get("pix_fmt")),
            ]
            for key, name, expected in checks:
                if expected is not None and video.get(key) != expected:
                    issues.append({
                        "param": name, "expected": str(expected),
                        "actual": str(video.get(key, "未知")), "severity": "错误",
                    })

        if audio and "audio" in preset_thresholds:
            a_std = preset_thresholds["audio"]
            checks = [
                ("codec", "音频编码", a_std.get("codec")),
                ("sample_rate", "采样率", a_std.get("sample_rate")),
                ("channels", "声道数", a_std.get("channels")),
            ]
            for key, name, expected in checks:
                if expected is not None and audio.get(key) != expected:
                    issues.append({
                        "param": name, "expected": str(expected),
                        "actual": str(audio.get(key, "未知")), "severity": "警告",
                    })
